...
<x-modals.popup modal="{{ $modal }}">
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div
            ...
            <button type="button"
                class="absolute top-3 end-2.5  bg-transparent  hover:text-heading rounded-base text-sm w-9 h-9 ms-auto inline-flex justify-center items-center"
            ...
            <div class="p-4 md:p-5 text-center">
                ...
                <h3 class="mb-6 ">Apakah anda yakin ingin menghapus kategori ini {{ $name }}? Kategori
                    yang sudah dihapus tidak dapat dikembalikan</h3>
                <div class="flex items-center space-x-4 justify-center">
                    <form action={{ route('category.destroy', $id) }} method="post">
                        @method('DELETE')
                        @csrf
                        <button data-modal-hide="popup-modal-{{ $modal }}" type="submit"
